...
class IssueViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        user = self.request.user  # This should be your custom User instance
...
class AssignIssueView(generics.UpdateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        project = issue.project  # Corrected to lowercase
        ...
        if not project.contributors.filter(id=assignee.id).exists():
            return Response({"detail": "User is not a contributor to this project."},
...
class UpdateIssueStatusView(generics.UpdateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({"detail": "Issue status updated successfully."}, status=status.HTTP_200_OK)